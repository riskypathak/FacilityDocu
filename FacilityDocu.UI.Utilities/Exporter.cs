...
namespace FacilityDocu.UI.Utilities
{
    public class Exporter
    {
        public static string Export(ProjectDTO project, string selectedExportFormat, string exportPath)
        {
            ...
            {
                outputFolderPath = ExportPdf(project, exportPath);
            }
        ...
        private static string ExportPdf(ProjectDTO project, string exportPath)
        {
            string outputFolderPath = Path.Combine(exportPath, $"{DateTime.Now.ToString("yyyyMMddhhmmss")}.pdf");
                ...
                for (int moduleIndex = 1; moduleIndex <= rig.Modules.Count; moduleIndex++)
                {
                    ...
                    for (int stepIndex = 1; stepIndex <= module.Steps.Count; stepIndex++)
                    {
                        ...
                        for (int actionIndex = 1; actionIndex <= step.Actions.Count; actionIndex++)
                        {
                            ...
                            menuRigHtml.Append(htmlLayout.ToString());
                        }
                    }
                }
            }
            ...
            htmlToPdfConverter.EvoInternalFileName = Path.GetFullPath("evointernal.dat");
            htmlToPdfConverter.LicenseKey = "52l5aHt7aHlofGZ4aHt5Znl6ZnFxcXE=";

            // Convert the HTML page given by an URL to a PDF document in a memory buffer
            byte[] outPdfBuffer = htmlToPdfConverter.GetPdfBytesFromHtmlString(menuRigHtml.ToString());
            File.WriteAllBytes(outputFolderPath, outPdfBuffer);
